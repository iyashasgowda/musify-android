...
class FolderList : AppCompatActivity(), OnCompletionListener, IControl, IService {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Thread {
            if (rv.adapter != null && rv.adapter!!.itemCount > 0) shufflePlay.setOnClickListener {
                Thread {
                    ...
                    for (folder in if (Utils.folders != null) Utils.folders else Utils.getFolders(context)) list.addAll(folder.songs)
                    shufflePlay.post {
                        if (list.size > 0) {
                            songs = list
                            shuffle = true
                            ...
                            engine.startPlayer()
                            mp!!.setOnCompletionListener(this@FolderList)
                            updateSnippet()
                            Utils.putShflStatus(context, true)
                            Toast.makeText(context, "Shuffle all songs", Toast.LENGTH_SHORT).show()
                        } else Toast.makeText(context, "No songs found in any folders :(", Toast.LENGTH_SHORT).show()
                    }
                }.start()
            }
        }.start()
        ...
        optionsBtn.setOnClickListener {
            ...
            listOption.visibility = View.GONE
            rescanMedia.setOnClickListener {
                ...
                Handler(Looper.getMainLooper()).postDelayed({
                    rv.adapter = FolderAdapter(context, Utils.getFolders(context), loader, notFound)
    ...
    private fun setIDs() {
        ...
        title.text = StringBuilder("Folders")
        ...
        if (Utils.folders == null || Utils.folders.size == 0) Handler(Looper.getMainLooper()).postDelayed(
            { rv.adapter = FolderAdapter(context, Utils.getFolders(context), loader, notFound) },
            10
        ) else rv.adapter = FolderAdapter(context, Utils.folders, loader, notFound)
