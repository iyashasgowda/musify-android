...
class AlbumList : AppCompatActivity(), OnCompletionListener, IControl, IService {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Thread {
            if (rv.adapter != null && rv.adapter!!.itemCount > 0) shufflePlay.setOnClickListener {
                ...
                val list = Utils.getAllSongsByCategory(context, "album $orderBy")
                if (list.size > 0) {
                    songs = list
                    shuffle = true
                    position = Random().nextInt(songs!!.size - 1 + 1)
                    engine.startPlayer()
                    mp!!.setOnCompletionListener(this@AlbumList)
                    updateSnippet()
                    Utils.putShflStatus(context, true)
                    Toast.makeText(context, "Shuffle all songs", Toast.LENGTH_SHORT).show()
                } else Toast.makeText(context, "No songs found in any albums :(", Toast.LENGTH_SHORT).show()
            }
        }.start()
        Thread {
        ...
        optionsBtn.setOnClickListener {
            ...
            listingOption.setOnClickListener {
                dialog.dismiss()
                val albumsSort = AlbumsSort(context, rv, loader, notFound)
                albumsSort.show(supportFragmentManager, null)
            }
        }
    }
    ...
    private fun setIDs() {
        ...
        title.text = StringBuilder("Albums")
