...
class AllSongList : AppCompatActivity(), OnCompletionListener, IControl, IService {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Thread {
            shufflePlay.setOnClickListener {
                if (rv.adapter != null && rv.adapter!!.itemCount > 0) if (Utils.getAllSongs(context).size > 0) {
                    shuffle = true
                    songs = Utils.getAllSongs(context)
                    position = Random().nextInt(songs!!.size - 1 + 1)
                    engine.startPlayer()
                    mp!!.setOnCompletionListener(this@AllSongList)
                    updateSnippet()
                    Utils.putShflStatus(context, true)
                    Toast.makeText(context, "Shuffle all songs", Toast.LENGTH_SHORT).show()
                } else Toast.makeText(context, "No songs found to play :(", Toast.LENGTH_SHORT).show()
            }
        }.start()
        Thread {
            sequencePlay.setOnClickListener {
                if (rv.adapter != null && rv.adapter!!.itemCount > 0) if (Utils.getAllSongs(context).size > 0) {
                    shuffle = false
                    songs = Utils.getAllSongs(context)
                    position = 0
        ...
        optionsBtn.setOnClickListener {
            ...
            rescanMedia.setOnClickListener {
                ...
                Handler(Looper.getMainLooper()).postDelayed({
                    rv.adapter = AllSongAdapter(context, Utils.getAllSongs(context), loader, notFound)
                    if (rv.adapter == null || rv.adapter!!.itemCount == 0) hideAttributes()
                }, 500)
            }
            listingOptions.setOnClickListener {
                dialog.dismiss()
                val allSongsSort = AllSongsSort(context, rv, loader, notFound)
                allSongsSort.show(supportFragmentManager, null)
            }
        }
    }
    ...
    private fun setIDs() {
        ...
        title.text = StringBuilder("All Songs")
