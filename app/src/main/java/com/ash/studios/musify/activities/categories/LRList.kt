...
class LRList : AppCompatActivity(), OnCompletionListener, IControl, IService {
        ...
        Thread {
            if (rv.adapter != null && rv.adapter!!.itemCount > 0) shufflePlay.setOnClickListener {
                if (Utils.getLR(context).size > 0) {
                    ...
                    engine.startPlayer()
                    mp!!.setOnCompletionListener(this@LRList)
                    updateSnippet()
                    Utils.putShflStatus(context, true)
                    Toast.makeText(context, "Shuffle all songs", Toast.LENGTH_SHORT).show()
                } else Toast.makeText(context, "No songs found in the Low-Rated :(", Toast.LENGTH_SHORT).show()
            }
        }.start()
        Thread {
        ...
        optionsBtn.setOnClickListener {
            ...
            rescanMedia.setOnClickListener {
                ...
                Handler(Looper.getMainLooper()).postDelayed({
                    rv.adapter = LRAdapter(context, Utils.getLR(context), loader, notFound)
                    if (rv.adapter == null || rv.adapter!!.itemCount == 0) hideAttributes()
                }, 500)
            }
            listingOptions.setOnClickListener {
                dialog.dismiss()
                val lrSort = LRSort(context, rv, loader, notFound)
                lrSort.show(supportFragmentManager, null)
            }
        }
    }
    ...
    private fun setIDs() {
        ...
        title.text = StringBuilder("Low Rated")
