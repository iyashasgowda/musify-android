...
class TRSort(private var context: Context, var rv: RecyclerView, var pb: ProgressBar, var nf: TextView) : BottomSheetDialogFragment() {
    ...
    var prefs: SharedPreferences = context.getSharedPreferences(TR_SORT, Context.MODE_PRIVATE)
    ...
    private fun setIDs(v: View) {
        ...
        button4.setOnCheckedChangeListener { compoundButton: CompoundButton?, checked: Boolean ->
            if (checked) {
                prefs.edit()
    ...
    private val data: Unit
        get() {
            rv.adapter = TRAdapter(context, Utils.getTR(context), pb, nf)
        }
