...
class TRList : AppCompatActivity(), OnCompletionListener, IControl, IService {
        ...
        Thread {
            if (rv.adapter != null && rv.adapter!!.itemCount > 0) shufflePlay.setOnClickListener {
                if (Utils.getTR(context).size > 0) {
                    shuffle = true
                    songs = Utils.getTR(context)
                    position = Random().nextInt(songs!!.size - 1 + 1)
                    engine.startPlayer()
                    mp!!.setOnCompletionListener(this@TRList)
                    updateSnippet()
                    Utils.putShflStatus(context, true)
                    Toast.makeText(context, "Shuffle all songs", Toast.LENGTH_SHORT).show()
                } else Toast.makeText(context, "No songs found in the Top-Rated :(", Toast.LENGTH_SHORT).show()
            }
        }.start()
        Thread {
        ...
        optionsBtn.setOnClickListener {
            ...
            rescanMedia.setOnClickListener {
                ...
                Handler(Looper.getMainLooper()).postDelayed({
                    rv.adapter = TRAdapter(context, Utils.getTR(context), loader, notFound)
                    if (rv.adapter == null || rv.adapter!!.itemCount == 0) hideAttributes()
                }, 500)
            }
            listingOptions.setOnClickListener {
                dialog.dismiss()
                val trSort = TRSort(context, rv, loader, notFound)
                trSort.show(supportFragmentManager, null)
            }
        }
    }
    ...
    private fun setIDs() {
        ...
        title.text = StringBuilder("Top Rated")
