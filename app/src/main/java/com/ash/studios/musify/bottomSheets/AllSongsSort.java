...
public class AllSongsSort extends BottomSheetDialogFragment {
    ...
    public AllSongsSort(Context context, RecyclerView rv, ProgressBar pb, TextView nf) {
        this.context = context;
        this.nf = nf;
        this.pb = pb;
        this.rv = rv;

        prefs = context.getSharedPreferences(ALL_SONGS_SORT, MODE_PRIVATE);
    }
    ...
    private void setIDs(View v) {
        ...
        button0.setOnCheckedChangeListener((compoundButton, checked) -> {
            if (checked) {
                prefs.edit()
    ...
    private void getData() {
        rv.setAdapter(new AllSongAdapter(context, Utils.getAllSongs(context), pb, nf));
    }
