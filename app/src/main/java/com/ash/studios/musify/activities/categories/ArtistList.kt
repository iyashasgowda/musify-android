...
class ArtistList : AppCompatActivity(), OnCompletionListener, IControl, IService {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Thread {
            if (rv.adapter != null && rv.adapter!!.itemCount > 0) shufflePlay.setOnClickListener {
                val orderBy = if (prefs.getBoolean("orderBy", false)) "desc" else "asc"
                val list = Utils.getAllSongsByCategory(context, "artist $orderBy")
                if (list.size > 0) {
                    songs = list
                    shuffle = true
                    position = Random().nextInt(songs!!.size - 1 + 1)
                    engine.startPlayer()
                    mp!!.setOnCompletionListener(this@ArtistList)
                    updateSnippet()
                    Utils.putShflStatus(context, true)
                    Toast.makeText(context, "Shuffle all songs", Toast.LENGTH_SHORT).show()
                } else Toast.makeText(context, "No songs found in any artists :(", Toast.LENGTH_SHORT).show()
            }
        }.start()
        Thread {
        ...
        optionsBtn.setOnClickListener {
            ...
            rescanMedia.setOnClickListener {
                ...
                Handler(Looper.getMainLooper()).postDelayed({
                    rv.adapter = ArtistAdapter(context, Utils.getArtists(context), loader, notFound)
                    if (rv.adapter == null || rv.adapter!!.itemCount == 0) hideAttributes()
                }, 10)
            }
            listingOptions.setOnClickListener {
                dialog.dismiss()
                val artistsSort = ArtistsSort(context, rv, loader, notFound)
                artistsSort.show(supportFragmentManager, null)
            }
        }
    }
    ...
    private fun setIDs() {
        ...
        title.text = StringBuilder("Artists")
        ...
        if (Utils.artists == null || Utils.artists.size == 0) Handler(Looper.getMainLooper()).postDelayed(
            { rv.adapter = ArtistAdapter(context, Utils.getArtists(context), loader, notFound) },
            10
        ) else rv.adapter = ArtistAdapter(context, Utils.artists, loader, notFound)
        if (rv.adapter == null || rv.adapter!!.itemCount == 0) hideAttributes()
