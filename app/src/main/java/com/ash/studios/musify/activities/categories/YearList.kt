...
class YearList : AppCompatActivity(), OnCompletionListener, IControl, IService {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Thread {
            if (rv.adapter != null && rv.adapter!!.itemCount > 0) shufflePlay.setOnClickListener {
                Thread {
                    ...
                    for (year in Utils.years) list.addAll(year.songs)
                    shufflePlay.post {
                        if (list.size > 0) {
                            songs = list
                            shuffle = true
                            ...
                            engine.startPlayer()
                            mp!!.setOnCompletionListener(this@YearList)
                            updateSnippet()
                            Utils.putShflStatus(context, true)
                            Toast.makeText(context, "Shuffle all songs", Toast.LENGTH_SHORT).show()
                        } else Toast.makeText(context, "No songs found in any years :(", Toast.LENGTH_SHORT).show()
                    }
                }.start()
            }
        }.start()
        ...
        optionsBtn.setOnClickListener {
            ...
            val setings = dialog.findViewById<ConstraintLayout>(R.id.settings)
            dialogName.text = title.text
            dialogIcon.setImageDrawable(icon.drawable)
            setings.visibility = View.GONE
            rescanMedia.setOnClickListener {
                ...
                Handler(Looper.getMainLooper()).postDelayed({
                    rv.adapter = YearAdapter(context, Utils.getYears(context), loader, notFound)
                    if (rv.adapter == null || rv.adapter!!.itemCount == 0) hideAttributes()
                }, 10)
            }
            listingOptions.setOnClickListener {
                dialog.dismiss()
                val yearsSort = YearsSort(context, rv, loader, notFound)
                yearsSort.show(supportFragmentManager, null)
            }
        }
    }
    ...
    private fun setIDs() {
        ...
        title.text = StringBuilder("Years")
        ...
        if (Utils.years == null || Utils.years.size == 0) Handler(Looper.getMainLooper()).postDelayed(
            { rv.adapter = YearAdapter(context, Utils.getYears(context), loader, notFound) },
            10
        ) else rv.adapter = YearAdapter(context, Utils.years, loader, notFound)
        if (rv.adapter == null || rv.adapter!!.itemCount == 0) hideAttributes()
